...
export async function GET(request: NextRequest) {
  ...
  const [destinationsResult, eventsResult] = await Promise.all([
    // Fetch a large pool of top destinations so we can randomly pick from them.
    supabase
      .from('destinations')
      .select('*')
  ...
  // Deduplicate then randomly pick from the pool to vary results on each request.
  const pool = deduplicateByName(destinationsResult.data ?? []);
  const uniqueDestinations = shuffleArray(pool).slice(0, destinationsLimit);
